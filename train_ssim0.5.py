...
args.save = 'train_ssim0.5-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
...
def train(train_queue, batches, args, model, ssim, mse_loss, optimizer, epoch):
    for batch in range(batches):
        ...
        for i in range(len(map_list)):
            ...
            ssimLoss += (1 - ssim(input1.unsqueeze(0), output.unsqueeze(0), normalize=True, val_range=1))*0.5 + (1 - ssim(input2.unsqueeze(0), output.unsqueeze(0), normalize=True, val_range=1))*0.5
